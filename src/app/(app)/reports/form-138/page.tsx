...
export default function ReportCardPage() {
  ...
  const query = useQuery(
    () => reportsApi.form138(activeId as ID),
    [activeId],
    { enabled: activeId != null },
  );
  const report = query.data?.data;

  const finals = report?.learning_areas.map(
    (a) => a.final ?? avg(a.quarters.filter((q): q is number => q != null)),
  );
  const generalAverage = finals && finals.length ? avg(finals) : 0;
  const remarks = generalAverage >= 75 ? "PASSED" : "FAILED";

  const totalDays = report?.attendance.reduce((s, m) => s + m.days, 0) ?? 0;
  const totalPresent = report?.attendance.reduce((s, m) => s + m.present, 0) ?? 0;
  const totalAbsent = report?.attendance.reduce((s, m) => s + m.absent, 0) ?? 0;
  ...
  return (
    ...
    >
      ...
      {activeId == null ? (
        <EmptyState
          icon={FileText}
          title="Look up a report card"
          description="Enter a student ID to generate their SF9."
        />
      ) : query.loading ? (
        <LoadingState rows={4} />
      ) : query.error ? (
        ...
        <EmptyState icon={FileText} title="No report card found" description="This student has no report card on file." />
      ) : (
        <>
          {/* Learner info */}
          <section className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-4">
            <Field label="Name of Learner" value={report.learner.name} className="col-span-2" />
            <Field label="LRN" value={report.learner.lrn ?? "—"} />
            <Field label="Sex" value={report.learner.sex ?? "—"} />
            <Field label="Grade & Section" value={`${report.learner.grade ?? "—"} – ${report.learner.section ?? "—"}`} />
            <Field label="School Year" value={report.learner.school_year ?? "—"} />
            <Field label="Adviser" value={report.learner.adviser ?? "—"} className="col-span-2" />
          </section>
          ...
          <section className="mt-7">
            ...
            <table className="w-full border-collapse text-[12px]">
              ...
              <tbody>
                {report.learning_areas.map((area, i) => {
          ...
          <Signatories
            items={[
              { name: report.learner.adviser ?? "—", role: "Class Adviser" },
